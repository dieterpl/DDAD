...
def trainer(model, category, config):
    ...
    task = Task.init(project_name="ddad", task_name="train")
    for epoch in range(config.model.epochs):
        for step, batch in enumerate(trainloader):
            ...
            task.logger.report_scalar(title="loss metrics", series="loss", value=loss, iteration=step)
            task.logger.report_scalar(title="epoch metrics", series="epoch", value=epoch, iteration=step)
    # Finish the task
    task.close()
